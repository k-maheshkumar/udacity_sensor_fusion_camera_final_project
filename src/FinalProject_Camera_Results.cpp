...
int main(int argc, const char *argv[])
{
    ...
    writeToFIle({" \n image index,detector type, total # of keypoints, descriptor type, TTC Lidar, TTC Camera\n"}, resultFileName);
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
    {
        ...
        string detectorType = argv[1]; // HARRIS, FAST, BRISK, ORB, AKAZE, SIFT
        ...
        writeToFIle(detectorType + "," + std::to_string(int(keypoints.size())) + ",", resultFileName);
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                {
                    ...
                    writeToFIle(std::to_string(ttcLidar) + ",", resultFileName);
                    ...
                    writeToFIle(std::to_string(ttcCamera) + ",", resultFileName);
